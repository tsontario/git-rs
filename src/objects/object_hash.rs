...
mod tests {
    ...
    fn test_build_object_hash() {
        ...
        let expected_hash =
            format!("{:x}", sha1::Sha1::digest(expected_hash_input.as_bytes())).to_string();
        ...
        zlib_encoder
            .write_all(expected_hash_input.as_bytes())
            .unwrap();
    ...
    fn test_build_object_bigger_than_buf_size() {
        ...
        let object_hash =
            ObjectHash::build_with_buf_size(&mut reader, &mut writer, obj_type, size, buf_size)
                .unwrap();
