...
fn test_cat_file_print_type() {
    ...
    let hash_object_out = Command::cargo_bin("my-git")
        .unwrap()
        .args(["-C", tempdir.path().to_str().unwrap(), "hash-object", "-w", tempfile.path().to_str().unwrap()])
        .assert()
        .success()
        .get_output().clone();
    ...
    let cat_file_out = Command::cargo_bin("my-git")
        .unwrap()
        .args([ "-C", tempdir.path().to_str().unwrap(), "cat-file", "-t", object_hash.as_str()])
        .assert()
        .success()
        .get_output().clone();

    let err = String::from_utf8(cat_file_out.stderr).unwrap();
    assert_eq!(String::from_utf8(cat_file_out.stdout).unwrap(), "blob\n".to_string());
}
...
fn test_cat_file_print_size() {
    ...
    let cat_file_out = Command::cargo_bin("my-git")
        .unwrap()
        .args([ "-C", tempdir.path().to_str().unwrap(), "cat-file", "-s", object_hash.as_str()])
        .assert()
        .success()
        .get_output().clone();

    let err = String::from_utf8(cat_file_out.stderr).unwrap();
    assert_eq!(String::from_utf8(cat_file_out.stdout).unwrap(), "11\n".to_string());
}
...
fn test_cat_file_print_content() {
    ...
    let cat_file_out = Command::cargo_bin("my-git")
        .unwrap()
        .args([ "-C", tempdir.path().to_str().unwrap(), "cat-file", "-p", object_hash.as_str()])
        .assert()
        .success()
        .get_output().clone();

    let err = String::from_utf8(cat_file_out.stderr).unwrap();
    assert_eq!(String::from_utf8(cat_file_out.stdout).unwrap(), "hello world\n".to_string());
    }
